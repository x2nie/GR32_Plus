...
type
  TMainForm = class(TForm)
    ...
    procedure ImgViewKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure ImgViewKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  ...
  begin
    begin
      ...
      try
        ...
        W := Bitmap.Width * 0.5;
        H := Bitmap.Height * 0.5;
        SetBounds(FloatPoint(P), FloatPoint(Bitmap.BoundsRect.BottomRight));
...
begin
  ...
  CreateNewImage( 400,300, clOrchid32);
...
{  Result := TTicLayer.Create(ImgView.Layers);
...
begin
  ...
  if Selection is TTicBitmapLayer then
    ...
    begin
      ...
      SetBounds(FloatPoint(0,0), FloatPoint(Bitmap.BoundsRect.BottomRight));
...
begin
  ...
  //TCustomImage32Access(ImgView).MouseMove(Shift, X, Y);
end;

procedure TMainForm.ImgViewKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  with ScreenToClient(Mouse.CursorPos) do
  //TCustomImage32Access(ImgView).MouseMove(Shift, X, Y);
...
procedure TMainForm.ResamplerClassNamesListChange(Sender: TObject);
var
  R: TCustomResampler;
begin
...
procedure TMainForm.SetKernelParameter(Kernel: TCustomKernel);
begin
